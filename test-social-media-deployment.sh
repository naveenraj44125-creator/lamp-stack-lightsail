...
# E2E Test: Deploy social-media-app using setup-complete-deployment.sh
# Copies the existing social-media-app and sets up deployment config/workflow
#
# Usage:
#   ./test-social-media-deployment.sh --source-dir /path/to/lamp-stack-lightsail
#   ./test-social-media-deployment.sh --source-dir /path/to/lamp-stack-lightsail --no-cleanup
...
TEST_REPO_NAME="social-media-test-$(date +%s)"
GITHUB_USERNAME=""
...
# Copy example-social-media-app from original workspace if not in source dir
ORIGINAL_WORKSPACE="/Users/naveenrp/Naveen/GIthub Actions in Lightsail/Cline"
if [[ ! -d "$SOURCE_DIR/example-social-media-app" ]]; then
    if [[ -d "$ORIGINAL_WORKSPACE/example-social-media-app" ]]; then
        echo -e "${BLUE}Copying example-social-media-app from original workspace...${NC}"
        rsync -av --exclude='node_modules' --exclude='.git' "$ORIGINAL_WORKSPACE/example-social-media-app" "$SOURCE_DIR/"
    else
        echo -e "${RED}Error: example-social-media-app not found in $SOURCE_DIR or $ORIGINAL_WORKSPACE${NC}"
        exit 1
    fi
fi
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   E2E Test: Social Media App Deployment                    ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# Step 2: Create test directory and copy social-media-app
echo -e "${BLUE}Step 2: Copying social-media-app to test directory...${NC}"

TEST_DIR="$SOURCE_DIR/test-social-media-$(date +%s)"
mkdir -p "$TEST_DIR"

# Copy social-media-app files (excluding node_modules)
rsync -av --exclude='node_modules' --exclude='.git' "$SOURCE_DIR/example-social-media-app/" "$TEST_DIR/"

echo -e "${GREEN}✓ Copied social-media-app to $TEST_DIR${NC}"
...
export APP_NAME="Social Media App"
export INSTANCE_NAME="social-media-instance"
export AWS_REGION=us-east-1
export BLUEPRINT_ID=ubuntu_22_04
export BUNDLE_ID=small_3_0
export DATABASE_TYPE=mysql
export DB_EXTERNAL=false
export DB_NAME=employee_social
...
create_deployment_config "nodejs" "Social Media App" "social-media-instance" "us-east-1" "ubuntu_22_04" "small_3_0" "mysql" "false" "" "employee_social" "" "" "" "false"
create_github_workflow "nodejs" "Social Media App" "us-east-1"
...
verify '[[ -d "frontend" ]]' "frontend/ directory exists"
verify '[[ -d "backend" ]]' "backend/ directory exists"
verify '[[ -d "database" ]]' "database/ directory exists"
...
# Commit all files
git add -A
git commit -m "Initial commit: Social Media App with GitHub Actions deployment"
...
if [[ "$SKIP_DEPLOY_WAIT" == "true" ]]; then
    ...
    echo -e "${BLUE}Waiting for workflow to complete (this may take several minutes)...${NC}"
...
if [[ "$WORKFLOW_STATUS" == "success" ]]; then
    ...
    # Get deployment URL
    INSTANCE_IP=$(aws lightsail get-instance --instance-name social-media-instance --query 'instance.publicIpAddress' --output text 2>/dev/null || echo "")
